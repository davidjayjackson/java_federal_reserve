<#
    build.ps1 - Assemble the FRED Java Calc add-in into build/FRED.oxt

    Pipeline:
      1. unoidl-write  : idl/**.idl              -> build/types/XFred.rdb   (UNO type library)
      2. javamaker     : build/types/XFred.rdb   -> build/gen/**.class      (Java bindings)
      3. javac         : src/**.java + bindings  -> build/classes/**.class
      4. jar           : classes + bindings      -> build/oxt/fred.jar      (+ RegistrationClassName)
...
Write-Host '[1/5] unoidl-write : idl -> build/types/XFred.rdb'
$rdb = Join-Path $typeDir 'XFred.rdb'
...
Write-Host '[2/5] javamaker    : build/types/XFred.rdb -> build/gen'
...
Write-Host '[4/5] jar          : classes + bindings -> build/oxt/fred.jar'
$fredJar = Join-Path $stage 'fred.jar'
$mf = Join-Path $root 'registration\MANIFEST.MF'
& $jar 'cfm' $fredJar $mf '-C' $clsDir '.' '-C' $genDir '.'
if ($LASTEXITCODE -ne 0) { throw "jar failed (exit $LASTEXITCODE)" }

# --- 5. Stage remaining package files and zip the .oxt -------------------
Write-Host '[5/5] zip           : staging -> build/FRED.oxt'
Copy-Item $rdb                                             (Join-Path $stage 'types\XFred.rdb')
...
Write-Host 'Install with:  unopkg add --force build\FRED.oxt   (see docs/INSTALL.md)'
