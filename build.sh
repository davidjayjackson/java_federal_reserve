#!/usr/bin/env bash
#
# build.sh - Assemble the FRED Java Calc add-in into build/FRED.oxt
#
# Pipeline:
...
rdb="$type_dir/XFred.rdb"
"$uw" "$types" "$root/idl" "$rdb"

# --- 2. Generate Java bindings from the type library ------------------------
echo '[2/5] javamaker    : build/types/XFred.rdb -> build/gen'
...
echo '[4/5] jar          : classes + bindings -> build/oxt/fred.jar'
fred_jar="$stage/fred.jar"
mf="$root/registration/MANIFEST.MF"
# Merge classes + bindings into one tree first: this JDK's `jar` tool rejects
# duplicate directory entries (e.g. com/example/fred/) when given as two
...
"$jar" cfm "$fred_jar" "$mf" -C "$jar_dir" .

# --- 5. Stage remaining package files and zip the .oxt ----------------------
echo '[5/5] zip          : staging -> build/FRED.oxt'
cp "$rdb" "$stage/types/XFred.rdb"
...
echo
echo "Built $oxt"
echo 'Install with:  unopkg add --force build/FRED.oxt   (see docs/INSTALL.md)'
